...
let userRoles = [{"email": "up730418@myport.ac.ukzz", "roles": ['user']}, {"email": "up730418@myport.ac.uk", "roles": ['user','admin']},];
...
app.get('/api/user/roles', function (req, res) {
	...
	let result = userRoles.filter((user) => {return user.email == req.user.emails[0].value});
